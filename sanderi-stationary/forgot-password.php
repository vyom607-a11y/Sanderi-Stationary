<?php
declare(strict_types=1);
require_once __DIR__ . '/db.php';
require_once __DIR__ . '/otp.php';
require_once __DIR__ . '/security.php';

$message = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    check_csrf();
    $mobile = normalize_mobile($_POST['mobile'] ?? '');
    if (strlen($mobile) !== 12) {
        $message = 'Enter a valid 10-digit mobile number.';
    } else {
        $query = db()->prepare('SELECT id, mobile FROM users WHERE mobile=? LIMIT 1');
        $query->execute([$mobile]);
        $user = $query->fetch();
        if (!$user) {
            $message = 'No account found with this email address.';
        } else {
            try {
                $otp = create_otp();
                db()->prepare('UPDATE users SET otp_hash=?, otp_expires_at=DATE_ADD(NOW(), INTERVAL 10 MINUTE) WHERE id=?')->execute([password_hash($otp, PASSWORD_DEFAULT), $user['id']]);
                if (!send_sms_otp($user['mobile'], $otp)) throw new RuntimeException('SMS provider rejected the request.');
                $_SESSION['reset_user_id'] = (int) $user['id'];
                header('Location: reset-password.php');
                exit;
            } catch (Throwable $error) {
                $message = $error->getMessage();
            }
        }
    }
}
?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forgot Password | Sanderi Stationary</title><link rel="stylesheet" href="style.css?v=9"><script src="app.js?v=8" defer></script></head><body><header class="site-header"><a class="brand" href="index.php">Sanderi <span>Stationary</span></a><nav class="nav"><a href="index.php">Home</a><a href="products.php">Shop</a><a href="login.php">Login</a><a href="register.php">Register</a></nav></header><main class="page-shell"><section class="form-card"><div class="eyebrow">Account recovery</div><h1>Forgot password?</h1><p>Enter your registered mobile number. We will send a reset OTP to it.</p><?php if ($message): ?><p class="alert"><?= htmlspecialchars($message) ?></p><?php endif; ?><form method="post"><input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>"><label>Mobile number<input name="mobile" placeholder="10 digit mobile number" inputmode="tel" required></label><button>Send reset OTP</button></form><p class="meta"><a href="login.php">Back to login</a></p></section></main></body></html>
